...
async function initializeTemplate(options: InitOptions) {
  ...
  const replacements: Record<string, string> = {
    '@tsports/termenv': packageName,
    'TypeScript port of termenv with 100% API compatibility': description!,
    'termenv': goPackageName,
    'https://github.com/muesli/termenv': goRepo,
    'https://github.com/TSports/termenv': repositoryUrl!,
    '"termenv", "go-port", "typescript"': keywords!.map(k => `"${k}"`).join(',\\n    ')
  };
